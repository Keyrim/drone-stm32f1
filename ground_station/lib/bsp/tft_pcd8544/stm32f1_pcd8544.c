...
void PCD8544_InitIO(void) {
	SPI_Init(PCD8544_SPI);

	/* Init WRX pin */
	BSP_GPIO_PinCfg(PCD8544_DC_PORT,PCD8544_DC_PIN, GPIO_MODE_OUTPUT_PP,GPIO_NOPULL,GPIO_SPEED_FREQ_MEDIUM);

	/* Init CS pin */
	BSP_GPIO_PinCfg(PCD8544_CE_PORT,PCD8544_CE_PIN, GPIO_MODE_OUTPUT_PP,GPIO_NOPULL,GPIO_SPEED_FREQ_MEDIUM);

	/* Init RST pin */
	BSP_GPIO_PinCfg(PCD8544_RST_PORT,PCD8544_RST_PIN, GPIO_MODE_OUTPUT_PP,GPIO_PULLUP,GPIO_SPEED_FREQ_LOW);

	//Reset pin HIGH
	HAL_GPIO_WritePin(PCD8544_RST_PORT, PCD8544_RST_PIN, 1);
...
void PCD8544_send(unsigned char data) {
	PCD8544_CE_LOW;
	SPI_WriteNoRegister(PCD8544_SPI,data);
	PCD8544_CE_HIGH;
}
	...
	switch (pin) {
		case PCD8544_Pin_DC:
			if (state != PCD8544_State_Low) {
				HAL_GPIO_WritePin(PCD8544_DC_PORT, PCD8544_DC_PIN, 1);
			} else {
				HAL_GPIO_WritePin(PCD8544_DC_PORT, PCD8544_DC_PIN, 0);
			}
			break;
		case PCD8544_Pin_RST:
			if (state != PCD8544_State_Low) {
				...
				HAL_GPIO_WritePin(PCD8544_RST_PORT, PCD8544_RST_PIN, 0);
			}
			break;
		default: break;
	}

		...
		switch(state_high_level){
				...
				switch(state_flying){
					...
					case FLYING:
						...
						if(channels[SWITCH_2] > 1650){
							//Rien pour le moment donc manuel par défault
			...
			case SEND_DATA :
				...
				if(channels[SWITCH_4] > 1500)
					sub_send_data(mpu_angles.z_acc, EVERY_IS_OK, gps.latitude_deg,gps.longitude_deg,mpu_angles.x, mpu_angles.y, channels[0], channels[1], channels[2], channels[3], channels[4], channels[5], channels[6], channels[7], v_bat, (uint8_t)state_flying, escs[0].pulsation, escs[1].pulsation, escs[2].pulsation, escs[3].pulsation);

...
#define NB_OCTECT_MAX 5	//On envoit au plus 10 octect par loop
...
void sub_send_data(double acc_z_, uint8_t every_is_ok_, double latitude_, double longitude_, double x, double y, int32_t ch1, int32_t ch2, int32_t ch3, int32_t ch4, int32_t ch5, int32_t ch6, int32_t ch7, int32_t ch8, double batterie_, uint8_t state, uint16_t m1, uint16_t m2, uint16_t m3, uint16_t m4){
	if(first_call){
		...
		radio1.nb_octet = 5 ;
		radio2.nb_octet = 5;
		radio1.periode  = 100 ; // 50
		radio2.periode	= 100 ; // 50
	...
	if(compteur % longitude.periode == 0 || longitude.to_send){
		if(compteur_octet >= longitude.nb_octet){
			TELEMETRIE_send_long(longitude_);
	...
	if(compteur % angles.periode == 0 || angles.to_send){
		if(compteur_octet >= angles.nb_octet){
			TELEMETRIE_send_angle_x_y_as_int(x, y);
	...
	if(compteur % moteurs.periode == 0 || moteurs.to_send ){
		if(compteur_octet >= moteurs.nb_octet){
			TELEMETRIE_send_moteur_all(m1, m2, m3, m4);
	...
	if(compteur % radio1.periode == 0 || radio1.to_send ){
		if(compteur_octet >= radio1.nb_octet){
			TELEMETRIE_send_channel_all_1_4(ch1, ch2, ch3, ch4);
	...
	if(compteur % radio2.periode == 0 || radio2.to_send ){
		if(compteur_octet >= radio2.nb_octet){
			TELEMETRIE_send_channel_all_5_8(ch5, ch6, ch7, ch8);
	...
	if(compteur % batterie.periode == 0 || batterie.to_send ){
		if(compteur_octet >= batterie.nb_octet){
			TELEMETRIE_send_v_bat(batterie_);
	...
	if(compteur % every_is_ok.periode == 0 || every_is_ok.to_send ){
			if(compteur_octet >= every_is_ok.nb_octet){
				TELEMETRIE_send_every_is_ok(every_is_ok_);
	...
	if(compteur % state_global.periode == 0 || state_global.to_send ){
		if(compteur_octet >= state_global.nb_octet){
			TELEMETRIE_send_state_flying(state);

...
static void _I2cFailRecover(){
    ...
    // Enable I/O
    BSP_GPIO_PinCfg(GPIOB, GPIO_PIN_8, GPIO_MODE_OUTPUT_OD, GPIO_PULLUP, GPIO_SPEED_FREQ_HIGH);
    BSP_GPIO_PinCfg(GPIOB, GPIO_PIN_9, GPIO_MODE_INPUT, GPIO_PULLUP, GPIO_SPEED_FREQ_HIGH);
...
int XNUCLEO53L0A1_SetIntrStateId(int EnableIntr, int DevNo){
    ...
    case 'l' :
        ...
        if( VL53L0A1_GPIO1_L_INTx == VL53L0A1_GPIO1_R_INTx || VL53L0A1_GPIO1_L_INTx == VL53L0A1_GPIO1_C_INTx){
        	debug_printf("Conflicting Exti for %d",DevNo);
            status = 1;
...
#endif
    default:
        debug_printf("Invalid DevNo %d",DevNo);
        status = -1;
        goto done;
    }
...
int XNUCLEO53L0A1_Init(void) {
    ...
    if (status != 0 || ExpanderData[0] != 0x00 || ExpanderData[1] != 0x16) {
        debug_printf("I2C Expander @0x%02X not detected",(int)I2cExpAddr0 );
        goto done_err;

    }
    status = _ExpanderRd( I2cExpAddr1, 0, ExpanderData, 2);
    if (status != 0 || ExpanderData[0] != 0x00 || ExpanderData[1] != 0x16) {
    	debug_printf("I2C Expander @0x%02X not detected",(int)I2cExpAddr1);
        goto done_err;
    }
    ...
    if (status) {
    	debug_printf("Set Expander @0x%02X DR", I2cExpAddr0);
        goto done_err;
    }
    ExpanderData[0] = 0xFF;
    ...
    if (status) {
    	debug_printf("Set Expander @0x%02X DR", I2cExpAddr1);
        goto done_err;
    }
...
//error with valid id
    if( status ){
    	debug_printf("expander i/o error for DevNo %d state %d ",DevNo, state);
    }
done:
    return status;
}
...
static int _ExpanderRd(int I2cExpAddr, int index, uint8_t *data, int n_data) {
    ...
    do {
    	...
    	status = I2C_ReadMulti(I2C1, I2cExpAddr, index, data,  n_data);
...
static int _ExpanderWR(int I2cExpAddr, int index, uint8_t *data, int n_data) {
    ...
    XNUCLEO53L0A1_GetI2cBus();
    status = I2C_WriteMulti(I2C1, I2cExpAddr, index, data,  n_data);

...
void mask_def_on_the_ground(Event_t * event){
	...
	MASK_set_flag(&event->mask_or[ON_THE_GROUND_MANUAL], FLAG_REQUEST_STOP_MOTORS);
	...
	MASK_set_flag(&event->mask_or[ON_THE_GROUND_MANUAL_ACCRO], FLAG_REQUEST_STOP_MOTORS);
...
void mask_def_manual_accro(Event_t * event){
	...
	MASK_set_flag(&event->mask_and[MANUAL_ACCRO_ON_THE_GROUND], FLAG_CHAN_9_OFF);
#if USE_BATTERIE_CHECK
